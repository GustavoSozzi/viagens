...
function Viagens() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                km_final: formData.km_final
                    ? parseInt(formData.km_final)
                    : null,
    ...
    const handleEdit = (viagem) => {
        setFormData({
            ...
            data_hora_final: viagem.data_hora_final
                ? viagem.data_hora_final.slice(0, 16)
                : "",
    ...
    const handleDelete = async () => {
        const id = deleteId;
        setDeleteId(null);
        console.log('id da exclusao: ' + id);

        try {
            setDeletingIds(prev => [...prev, id]);

            echo.channel(`viagens.${id}`).listen('.trip.delete.finished', (event) => {
                console.log('Viagem excluída com sucesso:', event);

                setViagens(prev => prev.filter(v => v.id !== id));
                setDeletingIds(prev => prev.filter(deletingId => deletingId !== id));

                echo.leave(`viagens.${id}`);
            });
    ...
    return (
        <div className="page">
            ...
            <div className="stats-grid">
                ...
                <div className="stat-card">
                    <h3>Em Andamento</h3>
                    <div className="value" style={{ color: "#f6ad55" }}>
                        {viagensEmAndamento}
                    </div>
                </div>
                <div className="stat-card">
                ...
                <div className="stat-card">
                    ...
                    <div className="value">
                        {totalKmPercorrido.toLocaleString("pt-BR")}
                    </div>
                </div>
            </div>
            ...
            {showForm && (
                <form onSubmit={handleSubmit} className="form-card">
                    ...
                    <div className="form-group">
                        <label>
                            Motoristas:{" "}
                            {formData.motorista_ids.length > 0 && (
                                <span style={{ color: "#667eea" }}>
                                    ({formData.motorista_ids.length} selecionado
                                    {formData.motorista_ids.length > 1
                                        ? "s"
                                        : ""}
                                    )
                                </span>
                        ...
                        <div className="checkbox-group">
                            {motoristas.map((motorista) => (
                                ...
                                >
                                    <input
                                        type="checkbox"
                                        checked={formData.motorista_ids.includes(
                                            motorista.id,
                                        )}
                                        onChange={() =>
                                            handleMotoristaToggle(motorista.id)
                                        }
                                    />
                                    {motorista.nome}
                                </label>
                        ...
                        <div className="form-group">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setFormData({
                                        ...formData,
                                        data_hora_inicial: e.target.value,
                                    })
                                }
                                required
                            />
                        ...
                        <div className="form-group">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setFormData({
                                        ...formData,
                                        data_hora_final: e.target.value,
                                    })
                                }
                                placeholder="Deixe vazio se em andamento"
                            />
                    ...
                    <table>
                        ...
                        {viagens.map((viagem) => (
                            <tr key={viagem.id}>
                                ...
                                <td>
                                        <span
                                            className={`status ${viagem.km_final ? "finalizada" : "andamento"}`}
                                        >
                                            {viagem.km_final
                                                ? "✓ Finalizada"
                                                : "⏱ Em Andamento"}
                                ...
                                <td className="actions">
                                    <button
                                        ...
                                        className="btn-edit"
                                        disabled={deletingIds.includes(viagem.id)}
                                    >
                                        Editar
                                    </button>
                                    <button
                                        onClick={() => setDeleteId(viagem.id)}
                                        className="btn-delete"
                                        disabled={deletingIds.includes(viagem.id)}
                                    >
                                        {deletingIds.includes(viagem.id) ? "Excluindo..." : "Excluir"}
                                    </button>
                                </td>
                            </tr>
                        ))}
